...
def construct_mask(row_exs: List, col_exs: List = None) -> torch.tensor:
    ...
    # exact match
    row_entity_ids = torch.tensor([entity_dict.entity_to_idx(ex.tail_id) for ex in row_exs], dtype=torch.int64)
    col_entity_ids = row_entity_ids if positive_on_diagonal else \
        torch.tensor([entity_dict.entity_to_idx(ex.tail_id) for ex in col_exs], dtype=torch.int64)
    # num_row x num_col
